...
merge :: Ord a => [a] -> [a] -> [a]
merge [] ys = ys -- Merging an empty list to a non-empty list
merge xs [] = xs
merge (x:xs) (y:ys)
  | x <= y = (x: merge xs (y:ys))
  | x > y  = (y: merge (x:xs) ys)
...
--   /  \
-- 3     2
-- -> (Node 1 (Node 3 Empty Empty) (Node 2 Empty Empty))
...
numEmpties :: Tree a -> Int
numEmpties Empty = 1
numEmpties (Node a t1 t2) = (numEmpties t1) + (numEmpties t2)

-- Count number of Node's in the tree
numNodes :: Tree a -> Int
numNodes Empty = 0
numNodes (Node a t1 t2) = 1 + (numNodes t1) + (numNodes t2)

-- Insert a new node in the leftmost spot in the tree
insertLeft :: a -> Tree a -> Tree a
insertLeft x Empty = Node x Empty Empty
insertLeft x (Node a t1 t2) = Node a (insertLeft x t1) t2
